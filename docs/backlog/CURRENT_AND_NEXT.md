...
**이름**: 프렌즈텔 MVP — 프렌즈홈 + 5개 게시판 + 글·댓글 + 프렌즈룸 + 키보드·공유

| 구분 | 내용 |
|------|------|
| **사용자 가치** | 링크 하나로 “우리만의 게시판”을 만들고, 친구와 공지·자유·소식·임시 게시판을 함께 사용 가능 |
| **포함** | 홈 생성, 홈별 5개 메뉴, 글쓰기/보기/댓글, 홈 메인 픽셀 프렌즈룸(유니크 작성자 실인원 반영), IT 모임 AI 프렌즈홈/5개 직군 페르소나 시드, 공지 비밀번호, 키보드 조작, URL 복사, GitHub Pages 배포 |
| **미포함** | 서버 저장, 로그인, 알림, 검색, 첨부파일 |
...
### 2.3 데이터·인프라
- **서버 연동**: 백엔드 API로 홈·글·댓글 저장·동기화 (멀티 디바이스)
